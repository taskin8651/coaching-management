...
<div class="admin-page-head">
    <div>
        ...
        <h2 class="admin-page-title">Edit Timetable</h2>

        <p class="admin-page-subtitle">
            Update class schedule, faculty, subject, room and timing
        </p>
    </div>
</div>
        ...
        <div class="form-card">
            <div class="form-card-header">
                ...
                <div>
                    ...
                    <p class="form-card-subtitle">Update branch, course, batch, subject and teacher</p>
                </div>
            </div>
                    ...
                    <div class="input-icon-wrap">
                        ...
                        <select name="branch_id" id="branch_id" class="field-input {{ $errors->has('branch_id') ? 'error' : '' }}">
                            ...
                            @foreach($branches as $id => $name)
                                <option value="{{ $id }}" {{ old('branch_id', $timetable->branch_id) == $id ? 'selected' : '' }}>
                                    {{ $name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="input-icon-wrap">
                        ...
                        <select name="batch_id" id="batch_id" required class="field-input {{ $errors->has('batch_id') ? 'error' : '' }}">
                            ...
                            @foreach($batches as $id => $name)
                                <option value="{{ $id }}" {{ old('batch_id', $timetable->batch_id) == $id ? 'selected' : '' }}>
                                    {{ $name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="input-icon-wrap">
                        ...
                        <select name="course_id" id="course_id" class="field-input {{ $errors->has('course_id') ? 'error' : '' }}">
                            ...
                            @foreach($courses as $id => $name)
                                <option value="{{ $id }}" {{ old('course_id', $timetable->course_id) == $id ? 'selected' : '' }}>
                                    {{ $name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="input-icon-wrap">
                        ...
                        <select name="subject_id" id="subject_id" class="field-input {{ $errors->has('subject_id') ? 'error' : '' }}">
                            ...
                            @foreach($subjects as $id => $name)
                                <option value="{{ $id }}" {{ old('subject_id', $timetable->subject_id) == $id ? 'selected' : '' }}>
                                    {{ $name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="input-icon-wrap">
                        ...
                        <select name="teacher_id" id="teacher_id" class="field-input {{ $errors->has('teacher_id') ? 'error' : '' }}">
                            ...
                            @foreach($teachers as $id => $name)
                                <option value="{{ $id }}" {{ old('teacher_id', $timetable->teacher_id) == $id ? 'selected' : '' }}>
                                    {{ $name }}
                                </option>
                            @endforeach
                        </select>
        ...
        <div class="form-card">
            <div class="form-card-header">
                ...
                <div>
                    ...
                    <p class="form-card-subtitle">Update class day, date, time and room</p>
                </div>
            </div>
                ...
                <div class="field-group">
                    ...
                    <div class="input-icon-wrap">
                        ...
                        <input type="date"
                               name="schedule_date"
                               id="schedule_date"
                               value="{{ old('schedule_date', $timetable->schedule_date ? \Carbon\Carbon::parse($timetable->schedule_date)->format('Y-m-d') : '') }}"
                ...
                <div class="field-group">
                    ...
                    <div class="input-icon-wrap">
                        ...
                        <select name="status" id="status" class="field-input {{ $errors->has('status') ? 'error' : '' }}">
                            ...
                            <option value="changed" {{ old('status', $timetable->status) == 'changed' ? 'selected' : '' }}>Changed</option>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    cascadeByParent(batchSelect, branchSelect, batchesByBranch, {
        ...
        keepValue: @json(old('batch_id', $timetable->batch_id)),
    });

    cascadeByParent(courseSelect, batchSelect, coursesByBatch, {
        placeholder: 'Optional',
        keepValue: @json(old('course_id', $timetable->course_id)),
    });

    cascadeByParent(subjectSelect, batchSelect, subjectsByBatch, {
        placeholder: 'Optional',
        keepValue: @json(old('subject_id', $timetable->subject_id)),
    });
});
</script>
@endsection