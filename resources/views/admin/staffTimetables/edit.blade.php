...
@section('page-title', 'Edit Duty Schedule')
...
<div class="admin-page-head">
    <div>
        ...
        <h2 class="admin-page-title">Edit Duty Schedule</h2>

        <p class="admin-page-subtitle">
            Update staff duty day/date, timing and location
        </p>
    </div>
</div>

<form method="POST" action="{{ route('admin.staff-timetables.update', $staffTimetable->id) }}">
    @csrf
    @method('PUT')
                    ...
                    <div class="input-icon-wrap">
                        ...
                        <select name="staff_id" id="staff_id" required class="field-input {{ $errors->has('staff_id') ? 'error' : '' }}">
                            @foreach($staffMembers as $id => $name)
                                <option value="{{ $id }}" {{ old('staff_id', $staffTimetable->staff_id) == $id ? 'selected' : '' }}>{{ $name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="input-icon-wrap">
                        ...
                        <select name="branch_id" id="branch_id" class="field-input {{ $errors->has('branch_id') ? 'error' : '' }}">
                            ...
                            @foreach($branches as $id => $name)
                                <option value="{{ $id }}" {{ old('branch_id', $staffTimetable->branch_id) == $id ? 'selected' : '' }}>{{ $name }}</option>
                            @endforeach
                        </select>
                    </div>
        ...
        <div class="form-card">
            ...
            <div class="form-card-body">
                    ...
                    <div class="input-icon-wrap">
                        ...
                        <select name="day_of_week" id="day_of_week" class="field-input {{ $errors->has('day_of_week') ? 'error' : '' }}">
                            ...
                            @foreach(['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'] as $day)
                                <option value="{{ $day }}" {{ old('day_of_week', $staffTimetable->day_of_week) == $day ? 'selected' : '' }}>{{ $day }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="field-group">
                    ...
                    <div class="input-icon-wrap">
                        ...
                        <input type="date"
                               name="schedule_date"
                               id="schedule_date"
                               value="{{ old('schedule_date', $staffTimetable->schedule_date ? \Carbon\Carbon::parse($staffTimetable->schedule_date)->format('Y-m-d') : '') }}"
                ...
                <div class="field-group">
                    ...
                    <div class="input-icon-wrap">
                        ...
                        <input type="time"
                               ...
                               id="start_time"
                               required
                               value="{{ old('start_time', \Carbon\Carbon::parse($staffTimetable->start_time)->format('H:i')) }}"
                ...
                <div class="field-group">
                    ...
                    <div class="input-icon-wrap">
                        ...
                        <input type="time"
                               ...
                               id="end_time"
                               required
                               value="{{ old('end_time', \Carbon\Carbon::parse($staffTimetable->end_time)->format('H:i')) }}"
                ...
                <div class="field-group">
                    ...
                    <div class="input-icon-wrap">
                        ...
                        <input type="text"
                               name="location"
                               id="location"
                               value="{{ old('location', $staffTimetable->location) }}"
                ...
                <div class="field-group">
                    ...
                    <div class="input-icon-wrap">
                        ...
                        <select name="status" id="status" class="field-input {{ $errors->has('status') ? 'error' : '' }}">
                            <option value="scheduled" {{ old('status', $staffTimetable->status) == 'scheduled' ? 'selected' : '' }}>Scheduled</option>
                            <option value="cancelled" {{ old('status', $staffTimetable->status) == 'cancelled' ? 'selected' : '' }}>Cancelled</option>
                        </select>
                    </div>
                ...
                <div class="field-group">
                    ...
                    <textarea name="remarks" id="remarks" rows="3" class="field-input {{ $errors->has('remarks') ? 'error' : '' }}">{{ old('remarks', $staffTimetable->remarks) }}</textarea>
